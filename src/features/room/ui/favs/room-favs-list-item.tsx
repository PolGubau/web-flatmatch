...
export function RoomFavsListItem({ room }: Props) {
  return (
    <li key={room.id} className="group ">
      <Link to={`/room/${room.id}`} className="group-hover:bg-foreground/10 bg-foreground/5 transition-opacity rounded-xl gap-4 grid grid-cols-[auto_1fr_auto] overflow-hidden">
        ...
        <footer className='flex '>
          ...
          <HugeiconsIcon icon={ArrowUpRight03Icon} size={20} className="mt-4 mr-4 group-hover:mt-3 group-hover:mr-3 transition-all ease-in-out" />
        </footer>
      </Link>
