...
export function GalleryForm({ onChangeImages, onChangeMain, images, mainIndex }: Props) {
	return (
		<div className="space-y-4">
			<input
				accept="image/*"
				className="bg-foreground/20 rounded-full py-2 px-4 w-fit"
				multiple
				onChange={(e) => {
				...
				{images?.map((image, i) => {
					...
					return (
						<li className="relative group" key={key}>
							...
							<button
								className="absolute text-red top-1 right-1 rounded-full bg-error/80 p-1 cursor-pointer hover:bg-error transition-all invisible group-hover:visible  "
								onClick={handleDelete}
								type="button"
							>
								<HugeiconsIcon icon={DeleteIcon} size={20} />
							</button>
						</li>
					);
				})}
